...
BOOL FileFunSecond::Fun_IsFileExist(CString sPathName_x, HANDLE* h) //파일 탐색 함수
{
...
BOOL FileFunSecond::Fun_Find_Dir_size(CString sPathName_x, WIN32_FIND_DATA* data) //파일 탐색 함수
{
...
void FileFunSecond::Fun_Read_Dir(char*& Read, WIN32_FIND_DATA data, HANDLE* h, wchar_t*& strUnicode2) //파일 탐색 함수
{
...
void FileFunSecond::Fun_Save_Dir(CString hFilePath, CString str) {
	cout << "FileSave" << endl;
...
BOOL FileFunSecond::x_FindFiles(LPCTSTR path)
{
...
BOOL FileFunSecond::x_FindFilePath(CString& path)
{
	// 파일이 이미 존재하는 경우, 해당 경로를 반환
	if (FileFunSecond::x_FindFiles(path)) {
		return TRUE;
	}
	...
	if (relativePath.Find(_T(':')) < 0 && relativePath.Find('\\') < 0 && relativePath.Find('/') < 0) {
		...
		if (FileFunSecond::x_FindFiles(CString(buffer) + _T('\\') + relativePath)) {
...
int FileFunSecond::x_Fun_Read_Dir(int*& p_ptr, void*& p, char*& my_c_p_ReadData, int mode, int* p_dll_ptr) {
...
static int* p_dll_ptr;
void FileFunSecond::FileOn(LPVOID x_po) {
	...
	int num = 0;
	CString  strpath2 = L"AbandonWareSetting2.ini";
	FileFunSecond::x_FindFilePath(strpath2);
	...
	FileFunSecond::Fun_Find_Dir_size(strpath2, &filedata2);//strpath의 경로 안에 filedata의 스트럭처 데이터를 집어넣는다. size와 잡다한 파일 날짜 등의 데이터를 가져온다.
	FileFunSecond::Fun_IsFileExist(strpath2, &hFile2);//strpath의 경로 안에 hFile에 hFile 크레이트파일아 데이터를 가져온다.
	//경로와 hFile핸들 정보를 가져온다. 핸들 정보는 &로 말아서 포인터처럼 리턴할떄 재반환 하게 준비한다. 
	FileFunSecond::Fun_Read_Dir(*&my_c_p_ReadData, filedata2, &hFile2, *&str_p_Unicode2);
	//해당 경로의 데이터의 정보를 메모리에 올리고 캐싱한다. 
	InTerrupt::intTrue(*&switch_on_off2); //비활성화 스위치 On
	 void* pointersize;
	p_dll_ptr = static_cast<int*>(static_cast<void*>(static_cast<int*>(x_po) + 10000));

	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, POINTER_MODE, p_dll_ptr);
	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, TEXT_MODE, p_dll_ptr);
	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, KEY_MODE_FRIST, p_dll_ptr);
	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, KEY_MODE_SECOND, p_dll_ptr);
	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, KEY_MODE_THIRD, p_dll_ptr);
	FileFunSecond::x_Fun_Read_Dir(p_ptr, pointersize, *&my_c_p_ReadData, KEY_MODE_FOURTH, p_dll_ptr);
